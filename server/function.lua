		...
		if target and target > 0 then
			...
			if ping and ping > 0 then
				if duree and duree < 365 then
					...
					TriggerEvent('hw_sqlban:sendMessage', source, Text.invalidtime)
				end	
			else
				TriggerEvent('hw_sqlban:sendMessage', source, Text.invalidid)
			end
		else
			TriggerEvent('hw_sqlban:sendMessage', source, Text.invalidid)
		end
	else
		TriggerEvent('hw_sqlban:sendMessage', source, Text.cmdban)
	end
end
...
function cmdunban(source, args)
	...
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE license like @playername', 
	{
		...
		if data[1] then
			if #data > 1 then
				TriggerEvent('hw_sqlban:sendMessage', source, Text.toomanyresult)
				for i=1, #data, 1 do
					TriggerEvent('hw_sqlban:sendMessage', source, data[i].license)
				end
			else
				MySQL.Async.execute(
				'DELETE FROM banlist WHERE license = @name',
				{
				  ['@name']  = data[1].license
					...
					TriggerEvent('hw_sqlban:sendMessage', source, data[1].license .. Text.isunban)
				end)
			end
		else
			TriggerEvent('hw_sqlban:sendMessage', source, Text.invalidname)
		end
...
function cmdbanoffline(source, args)
	if args ~= "" then
		...
		if duree ~= "" then
			...
			TriggerEvent('hw_sqlban:sendMessage', source, Text.invalidtime)
			TriggerEvent('hw_sqlban:sendMessage', source, Text.cmdbanoff)
		end
	else
		TriggerEvent('hw_sqlban:sendMessage', source, Text.cmdbanoff)
	end
	...
	local name   = table.concat(args, " ",1)
		if name ~= "" then
			if nombre and nombre > 0 then
				...
				TriggerEvent('hw_sqlban:sendMessage', source, (nombre .." : ".. resultat))
			else
				for i = 1, #BanListHistory, 1 do
					if (tostring(BanListHistory[i].targetplayername)) == tostring(name) then
						...
						TriggerEvent('hw_sqlban:sendMessage', source, (i .." : ".. resultat))
					end
				end
			end
		else
		...
		TriggerEvent('hw_sqlban:sendMessage', source, Text.cmdhistory)
	end
end
...
function ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE targetplayername like @playername', 
	{
		...
		if not data[1] then
			...
			if permanent == 0 then
				TriggerEvent('hw_sqlban:sendMessage', source, (Text.youban .. targetplayername .. Text.during .. duree .. Text.forr .. reason))
			else
				TriggerEvent('hw_sqlban:sendMessage', source, (Text.youban .. targetplayername .. Text.permban .. reason))
			end
			...
			TriggerEvent('hw_sqlban:sendMessage', source, (targetplayername .. Text.alreadyban .. reason))
		end
	end)
end
