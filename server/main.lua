...
CreateThread(function()
	while Config.MultiServerSync do
		...
		function (data)
			...
			loadBanListHistory()
			TriggerClientEvent('hw_sqlban:Respond', -1)
			end
		end
		)
	end
...
ESX.RegisterCommand('sqlbanreload', Config.Permission, function(source, args, user)
	...
	if BanListLoad == true then
	  TriggerEvent('hw_sqlban:sendMessage', source, Text.banlistloaded)
	  if BanListHistoryLoad == true then
		  TriggerEvent('hw_sqlban:sendMessage', source, Text.historyloaded)
	  end
	else
	  TriggerEvent('hw_sqlban:sendMessage', source, Text.loaderror)
	end
end, true)
...
--How to use from server side : TriggerEvent("hw_sqlban:ICheat", "Auto-Cheat Custom Reason",TargetId)
RegisterServerEvent('hw_sqlban:ICheat')
AddEventHandler('hw_sqlban:ICheat', function(reason,servertarget)
	local license,identifier,liveid,xblid,discord,playerip,target
	local duree     = 0
	local reason    = reason

	if not reason then reason = "You're busted, my brother..." end
	...
	if target and target > 0 then
		...
		if ping and ping > 0 then
			if duree and duree < 365 then
				...
				print("hw_sqlban Error : Auto-Cheat-Ban time invalid.")
			end	
		else
			print("hw_sqlban Error : Auto-Cheat-Ban target are not online.")
		end
	else
		print("hw_sqlban Error : Auto-Cheat-Ban have recive invalid id.")
	end
end)

RegisterServerEvent('hw_sqlban:CheckMe')
AddEventHandler('hw_sqlban:CheckMe', function()
	doublecheck(source)
end)
...
AddEventHandler('hw_sqlban:sendMessage', function(source)
	print('^0[^1SQL BAN RELOAD^0]: ^3Bans reloaded succesfully!^0')
end)
